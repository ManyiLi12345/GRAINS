...
class Fold(object):
        ...
        for arg in arg_lists:
            ...
            if isinstance(arg[0], Fold.Node):
                ...
                try:
                    ...
                    print u"Constructing LongTensor from %s" % unicode(arg)
                    raise
        return res
    ...
    def apply(self, nn, nodes):
            ...
            for op in self.steps[step]:
                ...
                try:
                    batched_args = self._batch_args(
                        izip(*self.steps[step][op]), values)
                except Exception:
                    print u"Error while executing node %s[%d] with args: %s" % (
        ...
        try:
            ...
            print u"Retrieving %s" % nodes
            for lst in nodes:
                if isinstance(lst, Fold.Node):
                    print u', '.join([unicode(x.get(values).size()) for x in lst])
            raise
...
class Unfold(object):
    ...
    class Node(object):
        ...
        def split(self, num):
            return [Unfold.Node(self.tensor[i]) for i in xrange(num)]
