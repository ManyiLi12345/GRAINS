...
for epoch in xrange(config.epochs):
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        if not config.no_plot:
            ...
            dyn_plot.update_plots(ydata={u'Total_loss':plot_total_loss, u'Reconstruction_loss':plot_recon_loss,u'KLDLoss':plot_kldiv_loss})
            iter_id += 1
    ...
    if (epoch+1) % config.save_network_every == 0:
        try:
            ...
            print u"tmp models saved!"
        except:
            print u"failed to save tmp models"
            raise
