...
class FoldExt(Fold):
    ...
    def add(self, op, *args):
        ...
        self.total_nodes += 1
        #print u"arg len:",len(args)
        if len(args)>1:
            if not all([isinstance(arg, (
                    Fold.Node, int, torch.Tensor, torch.FloatTensor, torch.LongTensor, Variable)) for arg in args]):
                raise ValueError(
                        u"All args should be Tensor, Variable, int or Node, got: %s" % unicode(args))
        else:
            if not isinstance(args[0], (
                    Fold.Node, int, torch.Tensor, torch.FloatTensor, torch.LongTensor, Variable)):
                raise ValueError(
                        u"arg should be Tensor, Variable, int or Node, got: %s" % unicode(args))
        ...
        for arg in arg_lists:
            ...
            if isinstance(arg[0], Fold.Node):
                ...
                try:
                    ...
                    print u"Constructing float tensor from %s" % unicode(arg)
                    raise
        return res