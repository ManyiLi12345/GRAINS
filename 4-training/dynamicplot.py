...
class DynamicPlot(object):
    def __init__(self, title, xdata, ydata):
        ...
        self.yline = {}
        for label, data, idx in izip(ydata.keys(), ydata.values(), xrange(len(ydata))): # In  python 3.x, xrange is replaced by range
            if len(xdata) != len(data):
                print u'DynamicPlot::Error: Dimensions of x- and y-data not the same (skipping).' # In python 3.x, the line to be printed should be inside common brackets
                continue
